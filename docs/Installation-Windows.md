...
ML-Agents 支持wandows10.当然它也有可能支持windows的其他版本，但没有经过测试。并且，ML-Agents也没有在windows虚拟机上测试过。
...
为了使用ML-agents，你需要安装python和一些下边列出的python包。这篇引导还涉及如何创建一个基于GPU的训练。V0.4版本的ML-Agents不要求GPU训练，其可能被将来的版本所需求。
